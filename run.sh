...
if [[ -f "$DIR/vibrant_list.txt" ]]; then
    rm "$DIR/vibrant_list.txt"
fi

export FILE_LIST="$DIR/vibrant_list.txt"
cd $DIR
...
if [[ $NUM_FILES -eq 0 ]]; then
  echo "No files found in $DIR, please correct config file. Job terminated."
  exit 1
fi
...
JOB_ID=`qsub $ARGS -v FILE_LIST,DIR,OUT_DIR,STDERR_DIR,STDOUT_DIR -N run_vibrant_evaluation -e "$STDERR_DIR" -o "$STDOUT_DIR" -J 1-$NUM_FILES $SCRIPT_DIR/run_analysis.sh`
